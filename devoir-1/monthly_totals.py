...
def pivot_months_pandas(data: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    # Group by name and date and sum the precipitations
    monthly = data.groupby(["name", "month"])["precipitation"].sum().round(1).reset_index()
    monthly_pivoted = monthly.pivot_table(index=["name"], columns="month", values="precipitation")

    # Group by name and date and count the number of observations/stations
    counts = data.groupby(["name", "month"])["station"].count().reset_index()
    counts_pivoted = counts.pivot_table(index=["name"], columns="month", values="station").astype(int)

    return monthly_pivoted, counts_pivoted
...
def compute_pairwise(df: pd.DataFrame, func: callable) -> pd.DataFrame:
    ...
    **chaque paire de lignes du dataframe**.
    ...
    distance_matrix = squareform(pdist(df.values, func))
...
def compute_pairwise_distances(df: pd.DataFrame) -> pd.DataFrame:
    ...
    coordinates = df[["name", "latitude", "longitude"]].drop_duplicates().set_index("name").sort_values("name")
    distance_df = compute_pairwise(coordinates, geodesic)
...
def correlation(u, v) -> float:
    ...
    # Filter out Nans and get valid ids
    idx_u = ~pd.isna(u)
    idx_v = ~pd.isna(v)
    idx = idx_u & idx_v
    ...
    std_u, std_v = u.std(), v.std()

    correlation = ((u - mean_u) * (v - mean_v)).mean() / (std_u * std_v)
...
def compute_pairwise_correlation(df: pd.DataFrame) -> pd.DataFrame:
    ...
    filtered_data = (
        df[["name", "date", "precipitation"]]
        .pivot_table(index=["name"], columns="date", values="precipitation")
        .sort_values("name")
    )
    correlation_matrix = compute_pairwise(filtered_data, correlation)
...
def compute_pairwise_correlation_pandas(df: pd.DataFrame) -> pd.DataFrame:
    ...
    filtered_data = df[["name", "date", "precipitation"]].pivot_table(
        index=["date"], columns="name", values="precipitation"
    )
...
def main():
    ...
    expected_output = pd.DataFrame(
        {"a": {"a": 0, "b": 1, "c": 1}, "b": {"a": 1, "b": 0, "c": np.sqrt(2)}, "c": {"a": 1, "b": np.sqrt(2), "c": 0}}
    )
