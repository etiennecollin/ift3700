...
def clean_data(df: pd.DataFrame, year_cutoff: int) -> pd.DataFrame:
    ...
    """
    # Enlever les rangées avec la date de naissance manquante
...
def bin_counts(df: pd.DataFrame, bins: list, bin_centers: list) -> pd.DataFrame:
    ...
    # Obtenir les bins
    bins = pd.cut(df["rating"], bins=bins, labels=bin_centers, right=True)

    # Grouper par les bins et obtenir les comptes de chaque bin
    hist = df.groupby(bins).size().reset_index(name="count")

    # Trier le df selon la valeur de "count" décroissante
    hist = hist.sort_values(by="count", ascending=False).reset_index(drop=True)

    # Ajouter la colonne des "count" normalisés
...
class PermutationTest:
    ...
    def job(self, seed: int = None) -> Tuple[int, int]:
        ...
        # Sample le groupe sur-représenté
        overrep_indices = shuffled_indices[: self.n_overrep]
        overrep_scores = self.df.loc[overrep_indices]

        # Sample le groupe sous-représenté
        underrep_indices = shuffled_indices[self.n_overrep : self.n_overrep + self.n_underrep]
        underrep_scores = self.df.loc[underrep_indices]

        # Obtenir le score maximum des groupes
        max_overrep = overrep_scores["rating"].max()
        max_underrep = underrep_scores["rating"].max()
...
def sample_two_groups(
    ...
    for i in tqdm(range(n_iter)):
        ...
        best_over.append(overrep_scores)
        best_under.append(underrep_scores)

    return np.array(best_over), np.array(best_under)


if __name__ == "__main__":
    # Testez votre code avec des exemples simples ici
    path = "./data/standard_oct22frl_xml.xml"
    df = parse_xml(path)
    print(df)
    df = clean_data(df, 2002)
    print(df)
    pt = PermutationTest(df, 2 * len(df) // 3, len(df) // 3)
    overrep_scores, underrep_scores = pt.job()
    print(overrep_scores, underrep_scores)

    x = pd.DataFrame({"rating": [1, 2, 4, 6, 6, 7, 8, 11]})
    bacs = [0, 5, 10, 15]
    bin_centers = [2.5, 7.5, 12.5]
    df = bin_counts(x, bacs, bin_centers)
    print(df)
